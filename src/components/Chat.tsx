...
const Chat: React.FC = () => {
    ...
    <div className="h-[100vh] flex flex-col overflow-hidden">
      <div className="flex justify-end m-4 gap-4">
        <img
          ...
          alt="Chat"
          className="w-10 h-10" // Adjust the size of the image
        />
  ...
  {filteredUsers.map((user, index) => {
    // Simulate whether the user has unread messages and the count
    const hasUnread = index % 2 === 0; // For demo, alternate users have unread messages
    const unreadCount = hasUnread ? Math.floor(Math.random() * 5) + 1 : null;
    const lastMessageTime = "10:30 AM"; // Static time for demonstration
  ...
  >
    <div
      className={`relative max-w-xs px-4 py-2 rounded-lg text-sm ${
        chat.sender === "You" ? "bg-blue-500 text-white" : "bg-gray-300 text-black"
      }`}
    >
      <div>
      ...
      <div className="flex items-center justify-end text-xs mt-1">
        <span
          className={`${
            chat.sender === "You" ? "text-blue-100" : "text-gray-500"
          }`}
        >
        ...
        {chat.sender === "You" && (
         ...
         <svg xmlns="http://www.w3.org/2000/svg" width="14" height="20" viewBox="0 0 14 20">
           <path d="M2,8 L5,11 L12,4" stroke="white" stroke-width="1.5" fill="none" />
           <path d="M2,12 L5,15 L12,8" stroke="white" stroke-width="1.5" fill="none" />
         </svg>
       </div>
